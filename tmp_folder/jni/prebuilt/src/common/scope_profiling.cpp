...
void ScopeTimer<Unit>::logElapsed(const double elapsedSec) const {
    const char* unitStr = [] {
        if constexpr (std::is_same_v<Unit, Timer::Nanoseconds>) {
            return "ns";
        } else if constexpr (std::is_same_v<Unit, Timer::Microseconds>) {
            return "us";
        } else if constexpr (std::is_same_v<Unit, Timer::Milliseconds>) {
            return "ms";
        } else {
            return "sec";
        }
